...
# Hardcoded API key (you can later move to environment variable)
API_KEY = "619f0dea8fc24a38beb115843251211"
BASE_URL = "https://api.weatherapi.com/v1"
...
def search_cities(query: str, limit: int = 6) -> List[Dict]:
    ...
    result = _make_request(url, params, is_search=True)
    formatted = result[:limit] if isinstance(result, list) else []
...
def get_current_weather(city: str) -> Dict:
    """Returns normalized current weather data for a city."""
    if not city or not _is_valid_input(city):
        return {"error": "Invalid city name."}
    ...
    if "error" not in raw:
        raw = _parse_current_weather(raw)

    _cache[cache_key] = raw
    return raw


def get_forecast_weather(city: str, days: int = 5) -> Dict:
    """Returns parsed forecast weather for the next days."""
    if not city or not _is_valid_input(city):
        return {"error": "Invalid city name."}
...
def _make_request(url: str, params: dict, is_search: bool = False, retries: int = 2) -> dict:
    """Handles HTTP request with retry and error handling."""
    if not API_KEY:
        return {"error": "API key missing."}
...
def _parse_current_weather(data: dict) -> dict:
    """Convert WeatherAPI current response to simplified format."""
    try:
        return {
            "city": data["location"]["name"],
            "region": data["location"]["region"],
